...
class HealthPredictor:
    ...
    def calculate_cpu_health(self, cpu_data):
        """
        CPU health based on temperature and usage patterns.
        - Safe zone: < 70°C → minimal degradation
        - Warning zone: 70–85°C → accelerated degradation  
        - Critical zone: > 85°C → rapid degradation (Arrhenius model)
        """
        temp = cpu_data.get("temperature", 50)
        usage = cpu_data.get("usage", 30)
        max_temp = cpu_data.get("max_temp", temp)

        # Temperature factor (major component)
        if temp <= 60:
            temp_score = 100
        elif temp <= 70:
            temp_score = 100 - (temp - 60) * 2  # 80-100
        elif temp <= 80:
            temp_score = 80 - (temp - 70) * 3   # 50-80
        elif temp <= 90:
            temp_score = 50 - (temp - 80) * 4   # 10-50
        else:
            temp_score = max(5, 10 - (temp - 90) * 2)

        # Usage factor (minor component — high sustained usage reduces life)
        usage_penalty = 0
        if usage > 80:
            usage_penalty = 10
        elif usage > 60:
        ...
        score = max(5, min(100, round(temp_score - usage_penalty)))

        # RUL calculation (months)
        # Based on Arrhenius equation approximation for semiconductor aging
        if temp <= 65:
            base_rul = 36  # 3 years at cool temps
        elif temp <= 75:
            base_rul = 24
        elif temp <= 85:
            base_rul = 14
        else:
            # Exponential decay for high temps
            base_rul = max(2, int(14 * math.exp(-(temp - 85) * 0.1)))

        # Adjust for usage intensity
        rul_months = max(1, int(base_rul * (1 - usage / 400)))
        ...
        return {
            ...
            "risk_factors": self._cpu_risk_factors(temp, usage, max_temp),
        }

    def _cpu_risk_factors(self, temp, usage, max_temp):
        factors = []
        if temp > 85:
            factors.append("Critical temperature — thermal throttling likely")
        elif temp > 70:
            factors.append("Elevated temperature — monitor cooling system")
        if usage > 80:
            factors.append("Sustained high CPU usage — consider workload optimization")
        if max_temp > 90:
            factors.append("Peak temperatures exceeded 90°C — risk of component damage")
        return factors
